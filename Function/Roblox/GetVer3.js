        ...
        client.RobloxVersion.findOne({ ID: 1 }, async (err,data) => {
            ...
            if (data) {
                if (data.CurrentVersion !== AppVersion) {
                    ...
                    client.channels.cache.get('837392652722110505').send(new MessageEmbed().setColor('RANDOM').setTitle('Roblox Updated!').setDescription('Roblox has updated! Please wait for your executor updated to the latest Version!').addField('Old Version', '``` [x] ' + data.OldVersion + '```' ).addField('Current Version', '``` [x] ' + AppVersion + '```'))
                }
